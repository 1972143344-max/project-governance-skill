...
This document defines the full reading protocol for non-trivial repository work.

Use it to decide:

- how to route before opening longer documents
- when probing is allowed
- when a summary or index read is no longer enough
...
1. external routing first
2. internal routing second
3. controlled probing last
4. governing expansion once the relevant material is known
5. stop once the action-ready authority set is loaded

## External Routing First

Prefer these routing surfaces before opening longer downstream files:
...
Use those surfaces to narrow what to open next.
...
- clear headings
- markers such as `purpose`, `authority`, `status`, `read this when`, `skip this when`

Section titles and keyword anchors are more robust than line-number routing. Use line numbers only as a convenience.

## Runtime Document Classes

After routing begins, classify the target surface and use the matching default first-read action.
